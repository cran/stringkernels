...
{  
  ...
  I_LCPFactory* lcp_fac = 0;
	...
	//	if(text[size-1] != SENTINEL)
	//   text = (SYMBOL*)(std::string(text)+SENTINEL).c_str();
	...
	sa_fac->ConstructSA(text, size, suftab);
	...
	// CW lcp_fac->ComputeLCP(text, size, suftab, lcptab);
	lcp_fac->ComputeLCP(text, size, (UInt32 *) suftab, lcptab);
...
#else
    /*  MK-Change: Don't use, this breaks things in mysterious ways:
        assert(right <= size-1) in ESA::GetSuflink */
...
ESA::~ESA()
{
  ...
  if(suflink)     { delete [] suflink; suflink=0; }
  if(suftab)      { delete [] suftab; suftab=0; }
  if(bcktab_val)  { delete [] bcktab_val; bcktab_val=0; }
  if(bcktab_key4) { delete [] bcktab_key4; bcktab_key4=0;}
  if(coef4)       { delete [] coef4; coef4 = 0; }
  if(bcktab_key8) { delete [] bcktab_key8; bcktab_key8=0;}
  if(coef8)       { delete [] coef8; coef8 = 0; }
}
...
ESA::ConstructChildTable(){
  ...
  UInt32 first0idx = 0;
	...
	// Loop thru and process each index. 
									...
									UInt32 &sl_i, UInt32 &sl_j)
{
  //' Variables
	...
	while(lcp < final_lcp) {
		GetIntervalByChar(lb,rb,ch,lcp,sl_i, sl_j);
		GetLcp(sl_i, sl_j, lcp);
								...
								UInt32 &sl_i, UInt32 &sl_j)
{
	...
	Compare(left, offset, &text[suftab[i]+1+offset], orig_lcp-1-offset, llcp); 
 	llcp += offset;

	if(llcp < orig_lcp-1) {
		Compare(right, offset, &text[suftab[i]+1+offset], orig_lcp-1-offset, rlcp); 
    rlcp += offset;
		...
		while(right-left > 1){
			mid = (left + right)/2;		
			 Compare(mid, c, &text[suftab[i]+1+c], orig_lcp-1-c, mlcp);
			mlcp += c;
								 ...
								 UInt32 &sl_i, UInt32 &sl_j)
{
  assert(child_i != child_j);
	...
	if(parent_i ==0 && parent_j == size-1) {  //' this is root interval
		...
		//' (2)
		lcp_parent = 0;
		 GetLcp(child_i,child_j,lcp_child);
		assert(lcp_child  >  0);
	}
	else {
		//' (1)
    GetSuflink(parent_i,parent_j,sl_i,sl_j);
    GetLcp(sl_i, sl_j, lcp_sl);

		//' (2)
    GetLcp(parent_i,parent_j,lcp_parent);
    GetLcp(child_i,child_j,lcp_child);
		assert(lcp_child > 0);
	}
    ...
    GetIntervalByChar(tmp_i, tmp_j, ch, lcp_sl, sl_i, sl_j);
...
ESA::ConstructSuflink()
{
  ...
  while (!q.empty()) {
		...
		do {
			...
			GetIntervalByIndex(interval.first, interval.second, start_idx, i, j); 
						
			if(j > i) {
				//' [i..j] is non-singleton interval
				FindSuflink(interval.first, interval.second, i,j, sl_i, sl_j);
											 ...
											 std::vector<std::pair<UInt32,UInt32> > &q)
{
	...
	do {
		assert(lb>=0 && rb<size);
		GetIntervalByIndex(lb,rb,k,i,j);
		//if(j-i> 0) {
    if(j > i) {    // chteo: saved 1 operation ;)  [260906]
												...
												const UInt32 &start_idx, UInt32 &child_i, UInt32 &child_j)
{
	...
	if(child_i == parent_i) {
		childtab.l_idx(parent_i,parent_j,child_j);
		child_j--;
											 ...
											 UInt32 &child_i, UInt32 &child_j)
{
	...
	childtab.l_idx(parent_i, parent_j, idx);
 ...
 *  \param j           - (IN) Right bound of the starting interval.
											...
											UInt32 &floor_len) 
{
    ...
    GetIntervalByChar(floor_lb, floor_rb, pattern[lcp], lcp, lb, rb);
	...
	while( (lb<=rb) && queryFound ) {
		if(lb != rb) {
			...
			floor_rb = rb;
			floor_len = lcp;
			GetIntervalByChar(floor_lb, floor_rb,pattern[matched_len],matched_len,lb,rb);
