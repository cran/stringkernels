...
#include <Rmath.h>
  ...
  SEXP wordkernel(SEXP rtext, // text document
		SEXP ltexts, // list or vector of text documents to compute kvalues against
		SEXP nltexts,  // number of text documents in ltext
		SEXP rntok,  // number of tokens in text
		SEXP lntoks, // tokens per document in ltext
		SEXP ktype,   // type of kernel
		SEXP param)   // parameter for kernel
  {
    ...
    // Construct ESASK
    UInt32 text_size = *INTEGER(rntok);
    int number_ltext = *INTEGER(nltexts);
    int *ltext_size =  (int *) malloc (sizeof(int) * number_ltext);
    memcpy(ltext_size, INTEGER(lntoks), number_ltext*sizeof(int));
    int weightfn = *INTEGER(ktype);
    const SYMBOL *text = (SYMBOL*) INTEGER(rtext);
    ...
    for (int i=0; i<number_ltext; i++)
      {
        const SYMBOL *pattern = (SYMBOL*) INTEGER(VECTOR_ELT(ltexts, i));
